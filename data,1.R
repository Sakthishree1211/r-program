...
dataset$Item<-gsub("UNKNOWN","smoothie",dataset$Item)
dataset$Item<-gsub("ERROR","juice",dataset$Item)
dataset$Quantity<-gsub("ERROR","4",dataset$Quantity)
dataset$Quantity<-gsub("UNKNOWN","2",dataset$Quantity)
...
dataset$Payment.Method<-gsub("UNKNOWN","cash",dataset$Payment.Method)
...
dataset$Transaction.Date<-gsub("ERROR","10/08/2023",dataset$Transaction.Date)
dataset$Transaction.Date<-gsub("UNKNOWN","12/02/2023",dataset$Transaction.Date)
...
str(dataset)
write.csv(dataset,"cafe__details.csv")
cafe=read.csv("cafe__details.csv")
View(cafe)
